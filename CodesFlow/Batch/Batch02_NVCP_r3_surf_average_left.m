...
% averaging surface measures across ages
% Dependencies: gifti (http://www.nitrc.org/projects/gifti/)
...
for m = 1 : 2
    ...
    for subi = 1 : length(files)
        ...
        if Birth_age >= 37
            subgii = gifti([files{subi},'/DHCP_template40/hemi-left_',measures{m},'.gii']);
            a = subgii.cdata;
...
roi_gii = gifti('~/VOTC34_L_binary.label.gii');
...
for m = 1 : 2
    ...
    for ai = 1 : 10
        % term
        tempt = data{m}{ai};
        output1 = [res_file,'/term/',measures{m},'/Age_',num2str(ai + 35),'_left_',measures{m},'.shape.gii'];
        if ~isempty(tempt)
        ...
        if ai ~= 10
        tempt = data_pre{m}{ai};
        output2 = [res_file,'/preterm/',measures{m},'/Age_',num2str(ai + 35),'_left_',measures{m},'.shape.gii'];
        if ~isempty(tempt)
            gii_mean = mean(tempt,2);
            subgii.cdata = gii_mean;
