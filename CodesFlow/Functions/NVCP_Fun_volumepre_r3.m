...
% Dependencies: DPABI (http://rfmri.org/dpabi)
%% prepare files
datainfo = load('~/SubInfo_r3.mat');
SubInfo = datainfo.subinfo;
temp_f = ['~/dhcp3/rel3_dhcp_fmri_pipeline','/sub-',SubInfo{subi,1},'/ses-',num2str(SubInfo{subi,2})];
% check the file
[FunFiles,FunNames] = my_ls([temp_f,'/func/*task-rest_desc-preproc_bold.nii.gz']);
FunTemt = [~/',SubInfo{subi,1},'/Func_tempt'];
if isfile([~/',SubInfo{subi,1},'/Func_volume/rfMRI_dcf_4D.nii'])
...
[Data,VoxSize,FileList,Header] = y_ReadAll(FunTemt); % I modified this function to make sure that the reading does not adjust orientation in affine matrix
load_tempt = load('~/rfMRI_crop.mat'); % the rfMRI crop data: choose 1600 continuous volume with lowest headmotion
crop_range = load_tempt.range{subi};
Data = Data(:,:,:,crop_range);
y_Write(Data,Header,FileList{1});
clear Data;
%% flirt to T2w space
FunVolume = [~/',SubInfo{subi,1},'/Func_volume'];
mkdir(FunVolume);
temp_f_ts{1} = ['~/DATA/dhcp3/rel3_dhcp_anat_pipeline',...
...
trans_mat = my_ls([temp_f,'/xfm/*from-bold_to-T2w_mode-image.mat']);
