...
% reg2surf
%% prepare files
[SubFiles,SubNames] = my_ls('resuts of Func in volume space');
...
%% surface files
SufFolder = ['~/rel3_dhcp_anat_pipeline/sub-',SubInfo{subi,1},'/ses-',num2str(SubInfo{subi,2}),'/anat'];% individual surface
hemi = {'left' 'right'};
...
FuncSurfFolder = ['~/',SubInfo{subi,1},'/Func_surf'];
...
temp_f{1} = ['~/DATA/dhcp3/rel3_dhcp_anat_pipeline/sub-',SubInfo{subi,1},'/ses-',num2str(SubInfo{subi,2})];
...
% smooth
    surfroi = ['~/',SubNames{subi},'/DHCP_template40/hemi-',...
